...
echo System %Cpu Cpu VSZ RSS Mem Swap Threads > rsyslogd_log.txt
echo System %Cpu Cpu VSZ RSS Mem Swap Threads

while [ True ]
do

	pid=`ps aux | grep rsyslogd | grep ? | awk '{print $2}'`
	proc=`pidstat -u -h -p $pid -r 1 1 | grep rsyslogd | awk '{print $5, $8, $9, $12, $13, $14}'`
