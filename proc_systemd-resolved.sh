...
echo System %Cpu Cpu VSZ RSS Mem Swap Threads > systemd_resolved_log.txt
echo System %Cpu Cpu VSZ RSS Mem Swap Threads

while [ True ]
do

	pid=`ps aux | grep systemd-resolved | grep ? | awk '{print $2}'`
	proc=`pidstat -u -h -p $pid -r 1 1 | grep systemd-resolved | awk '{print $5, $8, $9, $12, $13, $14}'`
