...
echo System %Cpu Cpu VSZ RSS Mem Swap Threads > systemd_udevd_log.txt
echo System %Cpu Cpu VSZ RSS Mem Swap Threads

while [ True ]
do

	pid=`ps aux | grep systemd-udevd | grep ? | awk '{print $2}'`
	proc=`pidstat -u -h -p $pid -r 1 1 | grep systemd-udevd | awk '{print $5, $8, $9, $12, $13, $14}'`
