...
echo System %Cpu Cpu VSZ RSS Mem Swap Threads > ovs_vswitchd_log.txt
echo System %Cpu Cpu VSZ RSS Mem Swap Threads

while [ True ]
do

	pid=`ps aux | grep ovs-vswitchd | grep ? | awk '{print $2}'`
	proc=`pidstat -u -h -p $pid -r 1 1 | grep ovs-vswitchd | awk '{print $5, $8, $9, $12, $13, $14}'`
